...
public class SwipeMenuLayout extends FrameLayout {
    ...
    public SwipeMenuLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (!isInEditMode()) {
            ...
            animDuration = a.getInteger(R.styleable.SwipeMenu_anim_duration, 100);
            a.recycle();
        }
    }
    ...
    public boolean onSwipe(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                int dis = (int) (mDownX - event.getX() * 2);
                if (state == STATE_OPEN) {
    ...
    public void smoothOpenMenu() {
        state = STATE_OPEN;
        if (mSwipeDirection == SwipeMenuRecyclerView.DIRECTION_LEFT) {
            mOpenScroller.startScroll(-mContentView.getLeft(), 0, mMenuView.getWidth() * 2, 0, animDuration);
        } else {
            mOpenScroller.startScroll(mContentView.getLeft(), 0, mMenuView.getWidth() * 2, 0, animDuration);
        }
        postInvalidate();
